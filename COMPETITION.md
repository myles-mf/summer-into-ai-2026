...
| 3 | Jun 29 → Jul 5, 2026 | **Data.Punk** — *"Go get that 'murica data."* Use real **data.gov** federal datasets; *"punk because the government never built apps for this data."* Volume over perfection. | **Entry #1: White Nose** (`week-3/`) — an animated dark map of **white-nose syndrome** spreading across America **county by county, winter by winter (2006–07 → 2018–19)**, built on **real USGS/USFWS occurrence data** (555 counties, 13 seasons, baked inline & self-contained). Press play and the white spreads out of one cave near Albany, NY; watch it **leap 1,300 miles to Washington State (2015–16)**, then take Texas, the Plains, California. Hover any county; pick your state to see when it reached you; "species being erased" panel (N. long-eared ↓97%, little brown/tricolored ↓90%, Indiana bat endangered); season-by-season narration. Theme = the purest *"the feds counted a catastrophe and never built anyone a way to see it"* punk. Data dug out of the USGS ArcGIS org (the obvious time-enabled service is stale at 2013-14; current county snapshot = `WNS_Counties_20191005`). Verified headlessly (`window.__wns.selfTest` passes: 555 counties / 474 US + 81 Canada / 45 states-provinces / 13 winters); no console errors; desktop + mobile. Dedicated to bat rehabbers ("for Natasha"). | ⬜ Built + wired into hub; pending commit/push + publish. |

| 4 | Jul 6 → Jul 12, 2026 | **Built on Yesterday** — the final week. Pick a prior submission — yours or a competitor's, from Summer *or* the earlier Spring Into AI sprint — and meaningfully extend it. | **Palace Radio** (`week-4/palace-radio/`) — rebuilds our **Spring Into AI** Method-of-Loci app (Memory Palace → Palace Radio) from scratch: an explorable **3D "Signal Station"** (Three.js beacon ring, additive glow + bloom, orbit + click-to-select, camera flythrough synced to narration) replaces the old flat photo/hotspot room; a real **server-generated Keeper voice** (OpenAI TTS via a Vercel API route) replaces browser `speechSynthesis`, enabling the feature the original build scoped and cut — **download the whole broadcast as one real audio file**, and a shareable link that sounds identical for every listener. Kept the numbers-station **Cipher** mode and **Spatial audio** (now derived from real 3D beacon coordinates) from the original; full "Signal Station" art-direction pass (Anton + JetBrains Mono, amber/teal phosphor palette, poster intro). Deployed on Vercel, same key-proxy pattern as `week-1/founding-father`, live at https://summer-into-ai-2026-zxrm.vercel.app/. | 🟨 Built + deployed + verified live; pending publish. |
