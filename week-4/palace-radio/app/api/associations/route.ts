...
If the user gave an explicit list, assign each list item to a locus in order and give exactly that many associations. If their request specifies an exact quantity (e.g. "five Spanish words for colors", "the first 5 elements", "10 things about the solar system"), give EXACTLY that many associations and stop there — do not pad or invent extra items just to fill every locus; it's completely fine to leave the remaining loci unclaimed. Only match the number of associations to the number of loci when the user gave a genuinely open-ended topic with no list and no stated count.${excludeClause}`
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { associations?: Array<{ locus: string; item: string; sentence: string; emoji?: string; color?: string }> }
